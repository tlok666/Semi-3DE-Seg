""" Full assembly of the parts to form the complete network """
import torch
import torch.nn.functional as F
from .unet_parts import *


class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, IsDiscriminator=False, bilinear=False):
        super(UNet, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = DoubleConv(n_channels, 64)
        self.down1 = Down(64, 128)
        self.down2 = Down(128, 256)
        self.down3 = Down(256, 512)
        self.down4 = Down(512, 512)
        self.up1 = Up(1024, 256, bilinear)
        self.up2 = Up(512, 128, bilinear)
        self.up3 = Up(256, 64, bilinear)
        self.up4 = Up(128, 64, bilinear)
        self.outc1 = OutConv(64, n_classes)
        
        self.outc2 = OutConv(64, n_classes)
        
        self.IsDiscriminator = IsDiscriminator
        self.mask_T = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.mask_T.data.fill_(0.2)
        
        self.Tan = torch.nn.Sigmoid()
        self.Sig = torch.nn.Sigmoid()

    def forward(self, x):
        x1 = self.inc(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        x5 = self.down4(x4)
        x = self.up1(x5, x4)
        x = self.up2(x, x3)
        x = self.up3(x, x2)
        x = self.up4(x, x1)
        #logits = self.outc(x)
        
        if self.IsDiscriminator:
            logits = self.outc2(x)
            logits = self.Sig(logits)
        else:
            logits = self.outc1(x)
            logits = self.Tan(logits)
        return logits
